...
int __cdecl main(int argc, char** argv)
{
    ...
    {
        std::perror("[-] failed to clear PiDDBCacheTable.\n");
        return -1;
    }
    ...
    if (!pool_base) {
        std::perror("[-] failed to allocate kernel pool\n");
        return -1;
    }
    ...
    std::printf("[+] DSE disabled for undetected loading.\n");
